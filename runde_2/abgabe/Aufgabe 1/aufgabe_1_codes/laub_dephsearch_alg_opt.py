...
#Unterschied zum originalem Greedy alg: Hier erlauben wir keine Züge, die auf leere Felder pusten. (siehe Doku)
def enumerate_moves():#enumeriert alle legitime Züge für den Schulhof
    ...
    for i in range(groessen[0]):
        for j in range(groessen[1]):#prüfe für alle Felder auf den der Hasumeister stehen könnte, ob die Rcihtung...
            if i+1<groessen[0] and schulhof[tuple(np.add([i,j],[1,0]) )] != 0:#obem
                moves.append([[i,j],[1,0]])
            if j+1<groessen[1] and schulhof[tuple(np.add([i,j],[0,1]) )] != 0:#rechts
                moves.append([[i,j],[0,1]])
            if i-1>=0 and schulhof[tuple(np.add([i,j],[-1,0]) )] != 0:#links
                moves.append([[i,j],[-1,0]])
            if j-1>=0 and schulhof[tuple(np.add([i,j],[0,-1]) )] != 0:#unten
                moves.append([[i,j],[0,-1]])
                                    #...okay ist. wenn ja, liste diese auf und...
    return moves#gib diese zurück
...
def pusten_predict(position, richtung, hof):# richtung: als vektor [a,b] angegeben a=0 oder b=0
    ...
    #print(position, richtung, ziel, ziel_links, ziel_rechts, neuesziel)
    ...
    if not istImHof(ziel):#wenn das Ziel nicht im hof ist!!! 50% werden einfach zur Seite geblasen
...
#enumeriere alle möglichen Züger zunächst
count = 0
while True:#wiederhole bis das programm beendet werden soll
    moves = enumerate_moves()
    info = greedy_step(schulhof)#solange das programm nicht beendet werden soll
    if info:
